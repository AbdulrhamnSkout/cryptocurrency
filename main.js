...
(function () {
    ...
    async function getBitcion() {
        try {
            data = await getJSON("https://api.coingecko.com/api/v3/coins/list")
            addCard(data)
        } catch (err) {
            alert(err)
    ...
    function addCard(data) {
        ...
        for (let i = 200; i < 300; i++) {
            let card = data[i]
            table.append(
                `<div  id="${card.id}_div" class="singla-currency card">
    ...
    function searchCoin() {
        const serachInput = $("#serach-input").val().trim()
        for (let i = 200; i < 300; i = i + 1) {
            let card = data[i]
            $(`#${card.id}_div`).show()
        }
        ...
        if (serachInput !== "") {
            ...
            for (let i = 200; i < 300; i = i + 1) {
                let card = data[i]
                const regex = new RegExp(`^${serachInput}$`);
                if (!regex.test(card.symbol)) {
                    {
                        $(`#${card.id}_div`).hide()
                        flag = flag - 1
                    }
            ...
            if (flag === 0) {
                setTimeout(() => {
                    alert("There is no coin bearing the name: " + serachInput + "\n Make sure you enter the name exactly as it appears on the card")
                }, 0)
            }
        }
    }
